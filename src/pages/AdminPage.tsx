...
import { AusgabenuebersichtViewDialog } from '@/components/dialogs/AusgabenuebersichtViewDialog';
import { EinnahmenuebersichtDialog } from '@/components/dialogs/EinnahmenuebersichtDialog';
import { EinnahmenuebersichtViewDialog } from '@/components/dialogs/EinnahmenuebersichtViewDialog';
...
const AUSGABENUEBERSICHT_FIELDS = [
  { key: 'ausgaben_kategorie', label: 'Kategorie der Ausgaben', type: 'lookup/select', options: [{ key: 'auto', label: 'Auto' }, { key: 'miete', label: 'Miete' }, { key: 'lebensmittel', label: 'Lebensmittel' }] },
  { key: 'ausgaben_betrag', label: 'Betrag der Ausgaben', type: 'number' },
  { key: 'ausgaben_beschreibung', label: 'Beschreibung der Ausgaben', type: 'string/text' },
  { key: 'neues_feld', label: 'recurring', type: 'applookup/select' },
  { key: 'datum', label: 'Datum', type: 'date/date' },
];
const EINNAHMENUEBERSICHT_FIELDS = [
  { key: 'einnahmen_betrag', label: 'Betrag der Einnahmen', type: 'number' },
  { key: 'datum', label: 'Datum', type: 'date/date' },
  { key: 'einnahmen_kategorie', label: 'Kategorie der Einnahmen', type: 'lookup/select', options: [{ key: 'gehalt', label: 'Gehalt' }, { key: 'bonus', label: 'Bonus' }] },
  { key: 'einnahmen_beschreibung', label: 'Beschreibung der Einnahmen', type: 'string/text' },
];

const ENTITY_TABS = [
  { key: 'ausgabenuebersicht', label: 'Ausgabenübersicht', pascal: 'Ausgabenuebersicht' },
  { key: 'einnahmenuebersicht', label: 'Einnahmenübersicht', pascal: 'Einnahmenuebersicht' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('ausgabenuebersicht');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'ausgabenuebersicht': new Set(),
    'einnahmenuebersicht': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'ausgabenuebersicht': return (data as any).ausgabenuebersicht as Ausgabenuebersicht[] ?? [];
      case 'einnahmenuebersicht': return (data as any).einnahmenuebersicht as Einnahmenuebersicht[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'ausgabenuebersicht': return AUSGABENUEBERSICHT_FIELDS;
      case 'einnahmenuebersicht': return EINNAHMENUEBERSICHT_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'ausgabenuebersicht': return {
        create: (fields: any) => LivingAppsService.createAusgabenuebersichtEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateAusgabenuebersichtEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteAusgabenuebersichtEntry(id),
      };
      case 'einnahmenuebersicht': return {
        create: (fields: any) => LivingAppsService.createEinnahmenuebersichtEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateEinnahmenuebersichtEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteEinnahmenuebersichtEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'ausgabenuebersicht' || dialogState?.entity === 'ausgabenuebersicht') && (
        <AusgabenuebersichtDialog
          open={createEntity === 'ausgabenuebersicht' || dialogState?.entity === 'ausgabenuebersicht'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'ausgabenuebersicht' ? handleUpdate : (fields: any) => handleCreate('ausgabenuebersicht', fields)}
          defaultValues={dialogState?.entity === 'ausgabenuebersicht' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Ausgabenuebersicht']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Ausgabenuebersicht']}
        />
      )}
      {(createEntity === 'einnahmenuebersicht' || dialogState?.entity === 'einnahmenuebersicht') && (
        <EinnahmenuebersichtDialog
          open={createEntity === 'einnahmenuebersicht' || dialogState?.entity === 'einnahmenuebersicht'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'einnahmenuebersicht' ? handleUpdate : (fields: any) => handleCreate('einnahmenuebersicht', fields)}
          defaultValues={dialogState?.entity === 'einnahmenuebersicht' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Einnahmenuebersicht']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Einnahmenuebersicht']}
        />
      )}
      {viewState?.entity === 'ausgabenuebersicht' && (
        <AusgabenuebersichtViewDialog
          open={viewState?.entity === 'ausgabenuebersicht'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'ausgabenuebersicht', record: r }); }}
        />
      )}
      {viewState?.entity === 'einnahmenuebersicht' && (
        <EinnahmenuebersichtViewDialog
          open={viewState?.entity === 'einnahmenuebersicht'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'einnahmenuebersicht', record: r }); }}
        />
      )}
