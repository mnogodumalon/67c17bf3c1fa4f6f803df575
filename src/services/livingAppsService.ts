...
import type { Ausgabenuebersicht, Einnahmenuebersicht, CreateAusgabenuebersicht, CreateEinnahmenuebersicht } from '@/types/app';
...
export class LivingAppsService {
  // --- AUSGABENUEBERSICHT ---
  static async getAusgabenuebersicht(): Promise<Ausgabenuebersicht[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.AUSGABENUEBERSICHT}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Ausgabenuebersicht[];
    return enrichLookupFields(records, 'ausgabenuebersicht');
  }
  static async getAusgabenuebersichtEntry(id: string): Promise<Ausgabenuebersicht | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.AUSGABENUEBERSICHT}/records/${id}`);
    const record = { record_id: data.id, ...data } as Ausgabenuebersicht;
    return enrichLookupFields([record], 'ausgabenuebersicht')[0];
  }
  static async createAusgabenuebersichtEntry(fields: CreateAusgabenuebersicht) {
    return callApi('POST', `/apps/${APP_IDS.AUSGABENUEBERSICHT}/records`, { fields: cleanFieldsForApi(fields as any, 'ausgabenuebersicht') });
  }
  static async updateAusgabenuebersichtEntry(id: string, fields: Partial<CreateAusgabenuebersicht>) {
    return callApi('PATCH', `/apps/${APP_IDS.AUSGABENUEBERSICHT}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'ausgabenuebersicht') });
  }
  static async deleteAusgabenuebersichtEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.AUSGABENUEBERSICHT}/records/${id}`);
  }

  // --- EINNAHMENUEBERSICHT ---
  static async getEinnahmenuebersicht(): Promise<Einnahmenuebersicht[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.EINNAHMENUEBERSICHT}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Einnahmenuebersicht[];
    return enrichLookupFields(records, 'einnahmenuebersicht');
  }
  static async getEinnahmenuebersichtEntry(id: string): Promise<Einnahmenuebersicht | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.EINNAHMENUEBERSICHT}/records/${id}`);
    const record = { record_id: data.id, ...data } as Einnahmenuebersicht;
    return enrichLookupFields([record], 'einnahmenuebersicht')[0];
  }
  static async createEinnahmenuebersichtEntry(fields: CreateEinnahmenuebersicht) {
    return callApi('POST', `/apps/${APP_IDS.EINNAHMENUEBERSICHT}/records`, { fields: cleanFieldsForApi(fields as any, 'einnahmenuebersicht') });
  }
  static async updateEinnahmenuebersichtEntry(id: string, fields: Partial<CreateEinnahmenuebersicht>) {
    return callApi('PATCH', `/apps/${APP_IDS.EINNAHMENUEBERSICHT}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'einnahmenuebersicht') });
  }
  static async deleteEinnahmenuebersichtEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.EINNAHMENUEBERSICHT}/records/${id}`);
  }
